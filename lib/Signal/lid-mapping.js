...
export class LIDMappingStore {
    ...
    async storeLIDPNMappings(pairs) {
        ...
        await this.keys.transaction(async () => {
            ...
            if (mappingKeys.length > BATCH_SIZE) {
                // Sort and remove oldest entries (keep most recent entries)
                mappingKeys.sort();
                const toRemove = mappingKeys.slice(0, mappingKeys.length - BATCH_SIZE);
                toRemove.forEach(k => delete mappingBatch[k]);
                this.logger.debug(`Cleaned up ${toRemove.length} old LID mappings (kept ${BATCH_SIZE})`);
            }
