...
import { BATCH_SIZE } from "../Defaults/index.js"
...
function signalStorage({ creds, keys }, lidMapping, logger) {
  ...
  return {
    ...
    storeSession: async (id, session) => {
      ...
      if (sessionKeys.length > BATCH_SIZE) {
        const recentSessions = sessionKeys.slice(-BATCH_SIZE)
        const trimmedBatch = {}
        recentSessions.forEach(key => {
          trimmedBatch[key] = sessionBatch[key]
        })
        logger?.debug?.(`[Signal] Trimmed session batch: kept ${BATCH_SIZE} most recent sessions`)
    ...
    storeSenderKey: async (senderKeyName, key) => {
      ...
      if (senderKeyIds.length > BATCH_SIZE) {
        // Sort by key name (typically timestamp-based) and remove oldest entries
        senderKeyIds.sort()
        const toRemove = senderKeyIds.slice(0, senderKeyIds.length - BATCH_SIZE)
        toRemove.forEach(k => delete senderKeyBatch[k])
        logger?.debug?.(`[Signal] Cleaned up ${toRemove.length} old sender keys (kept ${BATCH_SIZE})`)
      }
