...
import { BATCH_SIZE_SESSION, BATCH_SIZE_SENDER_KEY, BATCH_SIZE_DEVICE_LIST } from "../Defaults/index.js"
...
function signalStorage({ creds, keys }, lidMapping, logger) {
  ...
  return {
    ...
    storeSession: async (id, session) => {
      ...
      // Keep only the most recent sessions to prevent unlimited growth
      const sessionKeys = Object.keys(sessionBatch).sort()
      if (sessionKeys.length > BATCH_SIZE_SESSION) {
        const recentSessions = sessionKeys.slice(-BATCH_SIZE_SESSION)
        const trimmedBatch = {}
        recentSessions.forEach(key => {
          trimmedBatch[key] = sessionBatch[key]
        })
        logger?.debug?.(`[Signal] Trimmed session batch: kept ${BATCH_SIZE_SESSION} most recent sessions`)
        await keys.set({ session: { "_index": trimmedBatch } })
      } else {
        await keys.set({ session: { "_index": sessionBatch } })
      }
    },
    ...
    loadSenderKey: async senderKeyName => {
      try {
        ...
        // Load from batched sender-key storage
        const batchData = await keys.get("sender-key", ["_index"])
        const senderKeyBatch = batchData?.['_index'] || {}
        const key = senderKeyBatch[id]
    ...
    storeSenderKey: async (senderKeyName, key) => {
      ...
      // Store in batched sender-key storage
      const batchData = await keys.get("sender-key", ["_index"])
      const senderKeyBatch = batchData?.['_index'] || {}
      senderKeyBatch[id] = buf
      
      // Enforce batch size limit with cleanup of old entries
      const senderKeyIds = Object.keys(senderKeyBatch).filter(k => k !== "_index")
      if (senderKeyIds.length > BATCH_SIZE_SENDER_KEY) {
        // Sort by key name (typically timestamp-based) and remove oldest entries
        senderKeyIds.sort()
        const toRemove = senderKeyIds.slice(0, senderKeyIds.length - BATCH_SIZE_SENDER_KEY)
        toRemove.forEach(k => delete senderKeyBatch[k])
        logger?.debug?.(`[Signal] Cleaned up ${toRemove.length} old sender keys (kept ${BATCH_SIZE_SENDER_KEY})`)
      }
      
      await keys.set({ "sender-key": { "_index": senderKeyBatch } })
    },
