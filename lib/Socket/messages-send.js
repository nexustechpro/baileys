...
export const makeMessagesSocket = (config) => {
    ...
    const assertSessions = async (jids) => {
        ...
        for (const jid of uniqueJids) {
            ...
            if (cachedSession !== undefined) { if (cachedSession) continue } 
            else { const sessionValidation = await signalRepository.validateSession(jid); peerSessionsCache.set(signalId, sessionValidation.exists); if (sessionValidation.exists) continue }
        ...
        if (jidsRequiringFetch.length) {
            ...
            const result = await query({ tag: 'iq', attrs: { xmlns: 'encrypt', type: 'get', to: S_WHATSAPP_NET }, content: [{ tag: 'key', attrs: {}, content: wireJids.map(jid => ({ tag: 'user', attrs: { jid } })) }] })
    ...
    const getPrivacyTokens = async (jids) => {
        ...
        return await query({ tag: 'iq', attrs: { to: S_WHATSAPP_NET, type: 'set', xmlns: 'privacy' }, content: [{ tag: 'tokens', attrs: {}, content: jids.map(jid => ({ tag: 'token', attrs: { jid: jidNormalizedUser(jid), t, type: 'trusted_contact' } })) }] })
    }
