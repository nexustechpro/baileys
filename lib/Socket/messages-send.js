        ...
        if (result) {
            ...
            if (Object.keys(userDeviceUpdates).length > 0) {
                try {
                    ...
                    // Keep only 500 most recent users
...
const relayMessage = async (jid, message, { messageId: msgId, participant, additionalAttributes, additionalNodes, useUserDevicesCache, useCachedGroupMetadata, statusJidList } = {}) => {
    ...
    await authState.keys.transaction(async () => {
        ...
        // Handle group/status messages (NORMAL FLOW)
        if (isGroup || isStatus) {
            ...
            // Handle 1:1 messages (NORMAL FLOW)
            let ownId = meId
            if (isLid && meLid) {
                ownId = meLid
...
sendMessage: async (jid, content, options = {}) => {
    ...
    // ========================================
    // BYPASS FOR CUSTOM/UNRECOGNIZED MESSAGES
    // ========================================
    // If nexus cannot detect the type, it means it's a custom message
    // We should relay it directly without processing
    const messageType = nexus.detectType(content)
    
    if (!messageType && typeof content === 'object' && !('text' in content || 'image' in content || 'video' in content || 'audio' in content || 'document' in content || 'sticker' in content)) {
        // This is a custom message structure (like botInvokeMessage, raw proto, etc.)
        logger.debug({ content }, 'Bypassing nexus - sending custom message directly')
        
        try {
            const customMsg = await generateWAMessageFromContent(jid, content, {
                userJid,
                quoted,
                ...options
            })
            
            // For custom messages, skip group metadata fetching to avoid timeouts
            // Custom proto messages don't need encryption/group setup
            await relayMessage(jid, customMsg.message, { 
                messageId: customMsg.key.id,
                useCachedGroupMetadata: false, // Skip metadata fetch
                useUserDevicesCache: false, // Skip device enumeration
            ...
            if (config.emitOwnEvents) {
                process.nextTick(() => { 
                    processingMutex.mutex(() => upsertMessage(customMsg, 'append')) 
                })
            }
            
            return customMsg
        } catch (error) {
            logger.error({ error }, 'Failed to send custom message')
            throw error
        }
    }
    ...
    if (messageType) {
        return await nexus.processMessage(content, jid, quoted)
    }
    ...
    if (typeof content === 'object' && 'pin' in content) {
        ...
        await relayMessage(jid, pinMessage.message, { 
            messageId: pinMessage.key.id, 
