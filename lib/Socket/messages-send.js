...
import { DEFAULT_CACHE_TTLS, WA_DEFAULT_EPHEMERAL, BATCH_SIZE_SENDER_KEY_MEMORY, BATCH_SIZE_DEVICE_LIST } from '../Defaults/index.js'
        ...
        if (result) {
            ...
            if (Object.keys(userDeviceUpdates).length > 0) {
                try {
                    ...
                    const trimmedBatch = {}
                    userKeys.slice(-BATCH_SIZE_DEVICE_LIST).forEach(userId => { trimmedBatch[userId] = mergedBatch[userId] })
        ...
        await authState.keys.transaction(async () => {
            ...
            if (isGroup || isStatus) {
                ...
                if (isRetryResend) {
                    ...
                    if (memoryKeys.length > BATCH_SIZE_SENDER_KEY_MEMORY) {
                        // Sort and remove oldest entries (keep most recent entries)
                        memoryKeys.sort()
                        const toRemove = memoryKeys.slice(0, memoryKeys.length - BATCH_SIZE_SENDER_KEY_MEMORY)
                        toRemove.forEach(k => delete memoryBatch[k])
                        logger.debug(`Cleaned up ${toRemove.length} old sender-key-memory entries (kept ${BATCH_SIZE_SENDER_KEY_MEMORY})`)
                    }
