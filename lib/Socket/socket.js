...
import {
  ...
  MIN_UPLOAD_INTERVAL, NOISE_WA_HEADER, UPLOAD_TIMEOUT, BATCH_SIZE_DEVICE_LIST
...
export const makeSocket = (config) => {
  ...
  // Setup WebSocket URL with mobile check & routing info
  ...
  let epoch = 1, lastDateRecv, keepAliveReq, qrTimer, sessionHealthCheck, preKeyMonitorInterval, deviceListCleanupInterval, sessionCleanupInterval
  ...
  // Constants
  const CONSTANTS = { MAX_RECONNECT: 5, MAX_FAILED_PINGS: 6, PREKEY_CHECK_INTERVAL: 30 * 60 * 1000, PREKEY_MIN_INTERVAL: 5 * 60 * 1000, PREKEY_CRITICAL: 3 }
  const PRIORITY_MAP = { "signal-error": "critical", "bad-mac": "critical", "session-corruption": "critical", "auth-failure": "critical", "connection-established": "high", "connection-restored": "high", "device-paired": "high", "message-send-error": "normal", "message-received": "normal", "scheduled": "low", "keep-alive": "low" }

  // Core messaging - encode & send frame over WebSocket
  ...
  // Encode binary node & send via raw message
  ...
  // Wait for response message by ID with timeout
  ...
  // Query node with retry on rate limit
  ...
  // Execute USync query for contact/LID protocol
  const executeUSyncQuery = async (usyncQuery) => {
    ...
    const iq = {
      ...
      content: [{
        tag: "usync", attrs: { context: usyncQuery.context, mode: usyncQuery.mode, sid: generateMessageTag(), last: "true", index: "0" },
        content: [
  ...
  // Get phone numbers from LID USync - for contact sync
  ...
  // Handle unexpected errors - bad MAC & session corruption
  const onUnexpectedError = (err, msg) => {
    logger.error({ err }, `unexpected error in '${msg}'`)
    const message = (err && ((err.stack || err.message) || String(err))).toLowerCase()
    if (message.includes('bad mac') || (message.includes('mac') && message.includes('invalid'))) {
      logger.warn("Bad MAC error - pre-keys may be out of sync")
  ...
  // Validate auth state integrity before connection
  const validateAuthStateIntegrity = async () => {
    try {
      ...
      const preKeyData = currentPreKeyId > 0 ? await keys.get("pre-key", [currentPreKeyId.toString()]) : {}
      const hasCurrentPreKey = !!preKeyData[currentPreKeyId?.toString()]
      logger.info({ hasDeviceList, hasSessions, hasCurrentPreKey, currentPreKeyId }, "Auth state integrity check")
      if (!hasDeviceList) logger.warn("⚠️ device-list is empty - will rebuild from first messages")
      if (!hasSessions) logger.warn("⚠️ Sessions are empty - will establish as contacts message")
      if (!hasCurrentPreKey && currentPreKeyId > 0) logger.warn({ currentPreKeyId }, "⚠️ Current pre-key missing - will regenerate batch")
      logger.info("Auth state validation complete")
    } catch (error) {
      logger.error({ error }, "Auth state integrity check failed")
    }
  }

  // Validate WebSocket connection & perform handshake
  ...
  // Wait for next message frame with optional send
  ...
  // Get pre-key count from server
  const getAvailablePreKeysOnServer = async () => {
    const result = await query({
      tag: "iq", attrs: { id: generateMessageTag(), xmlns: "encrypt", type: "get", to: S_WHATSAPP_NET },
      content: [{ tag: "count", attrs: {} }]
    })
    return +getBinaryNodeChild(result, "count").attrs.value
  }

  // Upload pre-keys with retry logic & backoff
  ...
  // Smart pre-key monitor with priority queueing
  ...
  // Trigger pre-key check with priority mapping
  ...
  // Start background pre-key monitor interval
  ...
  // Stop background pre-key monitor
  ...
  // Upload pre-keys on connection if required
  const uploadPreKeysToServerIfRequired = async () => {
    try {
      ...
      if (!currentPreKeyExists && currentPreKeyId > 0) {
        logger.warn({ currentPreKeyId }, "🚨 Current pre-key file missing - regenerating FULL batch (95)")
      ...
      if (preKeyCount < MIN_PREKEY_COUNT) {
        ...
        logger.info(`Server pre-key count low (${preKeyCount}), uploading ${uploadCount} to reach initial count of ${INITIAL_PREKEY_COUNT}`)
        await uploadPreKeys(uploadCount)
      } else {
        logger.info(`✅ PreKey validation passed - Server: ${preKeyCount} pre-keys, Current ID: ${currentPreKeyId} exists: ${currentPreKeyExists}`)
      }
    } catch (error) {
  ...
  const cleanupSessionFiles = async () => {
    try {
      ...
      if (batchSize > 1000) {
        logger.info(`Session batch size is ${batchSize}, trimming to 1000 most recent`)
        ...
        logger.info(`✅ Trimmed session batch: removed ${deleted} old entries (kept 1000)`)
      } else {
  ...
  // Start session cleanup interval (every 10 minutes)
  const startSessionCleanup = () => {
    ...
    logger.info("Started session cleanup (every 10 minutes)")
  }
  ...
  const stopSessionCleanup = () => {
    if (sessionCleanupInterval) {
      ...
      logger.debug("Stopped session cleanup")
    }
  }

  // Handle incoming message frame - decode & emit appropriate events
  ...
  // Clean connection close - stop monitors & emit update
  const end = (error) => {
    if (closed) {
  ...
  const attemptReconnection = async (reason = "unknown") => {
    ...
    if (reconnectAttempts >= CONSTANTS.MAX_RECONNECT) {
      logger.error({ attempts: reconnectAttempts, maxAttempts: CONSTANTS.MAX_RECONNECT }, "Max reconnection attempts reached")
      end(new Boom("Connection Lost", { statusCode: DisconnectReason.connectionLost }))
      return
    }

    reconnectAttempts++
    // For network timeouts, use longer delays to wait for network recovery
  ...
  // Start keep-alive ping with failure tracking
  ...
  // Monitor session health for extended inactivity
  ...
  // Send passive IQ for connection keep-alive
  ...
  // Generate encrypted pairing key
  ...
  ws.on("close", (code, reason) => {
    ...
    if (!closed) {
      const delayMs = code === 1000 ? 1000 : 2000  // Longer delay for abnormal closes
      setTimeout(() => {
    ...
    if (node.attrs.lid && authState.creds.me?.id) {
      ...
      process.nextTick(async () => {
        try {
          ...
          if (deviceKeys.length > BATCH_SIZE_DEVICE_LIST) {
            // Sort and remove oldest entries (keep most recent)
            deviceKeys.sort()
            const toRemove = deviceKeys.slice(0, deviceKeys.length - BATCH_SIZE_DEVICE_LIST)
            toRemove.forEach(k => delete currentBatch[k])
            logger.debug(`Cleaned up ${toRemove.length} old device-list entries (kept ${BATCH_SIZE_DEVICE_LIST})`)
          }
...
// Map WebSocket error to Boom error
