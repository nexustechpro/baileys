...
// Enhanced encoder with no size limits
export const encodeWAMessage = (message) => {
    try {
        ...
        // If standard encoding fails, use manual field-by-field encoding with unlimited buffers
        console.log('Standard encoding failed, using unlimited buffer encoder:', error.message);
        
        // Increase protobuf buffer limits
        const writer = proto.Writer.create();
        try {
            proto.Message.encode(message, writer);
            const encoded = writer.finish();
            return writeRandomPadMax16(encoded);
        } catch (e) {
            // Last resort: bypass protobuf size checks
            const buffer = Buffer.allocUnsafe(1024 * 1024 * 100); // 100MB buffer
            const writer2 = proto.Writer.create(buffer);
            try {
                proto.Message.encode(message, writer2);
                const length = writer2.len;
                const result = buffer.slice(0, length);
                return writeRandomPadMax16(result);
            } catch (e2) {
                console.error('All encoding attempts failed:', e2);
                throw error;
            }
        }
    }
...
export function encodeNewsletterMessage(message) {
    try {
        ...
        // If standard encoding fails, use unlimited buffer
        console.log('Newsletter encoding failed, using unlimited buffer:', error.message);
        const buffer = Buffer.allocUnsafe(1024 * 1024 * 100); // 100MB buffer
        const writer = proto.Writer.create(buffer);
        try {
            proto.Message.encode(message, writer);
            const length = writer.len;
            return buffer.slice(0, length);
        } catch (e) {
            console.error('Newsletter encoding failed:', e);
            throw error;
        }
    }
}
